...
const themes = [
  { name: 'روشن', value: 'light', icon: LightIcon },
  { name: 'تاریک', value: 'dark', icon: DarkIcon },
  { name: 'سیستم', value: 'system', icon: SystemIcon },
]
...
export function ThemeSelector(props) {
  ...
  return (
    <Listbox as="div" value={theme} onChange={setTheme} {...props}>
      ...
      <ListboxOptions className="absolute left-1/2 top-full mt-3 w-36 -translate-x-1/3 space-y-1 rounded-xl bg-white p-3 text-sm font-medium shadow-md shadow-black/5 ring-1 ring-black/5 dark:bg-slate-800 dark:ring-white/5">
        {themes.map((theme) => (
          <ListboxOption
            key={theme.value}
            value={theme.value}
          ...
          >
            {({ selected }) => (
              <>
                ...
                <div className="mr-3">{theme.name}</div>
              </>
            )}
          </ListboxOption>
        ))}
